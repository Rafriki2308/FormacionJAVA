...
public class CustomAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        ...
        return authenticationManager.authenticate(authenticationToken); //We authenticate the token to check if the username data is valid
    }

    //Executed if authentication is successful to give the username his JWT token
    @Override
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authentication) throws IOException, ServletException {
        User username = (User) authentication.getPrincipal(); //returns the username that has successfully authenticated

        Algorithm algorithm = Algorithm.HMAC256("secret".getBytes()); //Create hash algorithm

        String access_token = JWT.create()
                .withSubject(username.getUsername()) //data to create the token from
                ...
                .withClaim("roles", username.getAuthorities().stream().map(GrantedAuthority::getAuthority).toList()) //link the roles to the token
